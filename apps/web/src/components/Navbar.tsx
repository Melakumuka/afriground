...
export default function Navbar({ currentLocale }: { currentLocale: string }) {
  ...
  const primaryDesktopLinks = [
    ...
    {
      href: `/${currentLocale}/station`,
      label: nav.network || (isZh ? "地面站网络" : "Network"),
    },
    {
      href: `/${currentLocale}/operations`,
  ...
  return (
    <header className="sticky top-0 z-50 w-full select-none">
      {/* Top Navbar Bar */}
      <div className="w-full bg-graphite/95 backdrop-blur-xl border-b border-graphite-600/70">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          >
            <div className="w-10 h-10 bg-signal flex items-center justify-center group-hover:scale-105 transition-transform rounded-sm shadow-md shadow-signal/20">
              <svg
                width="22"
                height="22"
                viewBox="0 0 24 24"
              ...
              >
                ...
                <path
                  ...
                  strokeLinecap="round"
                />
                <circle cx="12" cy="19.5" r="1.4" fill="#15171A" />
              </svg>
            </div>
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-mono text-steel-2 tracking-widest uppercase">
                GSaaS Platform
              </span>
            </div>
          </Link>
            ...
            {primaryDesktopLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`px-3.5 py-2 text-sm font-medium transition-all rounded-md ${
                    isActive
                      ? "bg-signal/15 text-signal-soft border border-signal/40 font-semibold"
          ...
          {/* Desktop Right Actions (Visible on lg+) */}
          <div className="hidden lg:flex items-center gap-3">
            {/* Status Indicator */}
            <div className="flex items-center gap-2 px-3 py-1.5 border border-graphite-600/80 bg-graphite-800/60 rounded-md text-xs text-steel-2">
              ...
              <span className="font-mono text-green-soft font-semibold text-[11px] whitespace-nowrap">
                {nav.nodes_online || (isZh ? "14/14 网络节点在线" : "14/14 Online")}
              </span>
            </div>
            ...
            <button
              ...
              className="px-2.5 py-1.5 border border-graphite-600 text-steel-2 hover:text-white hover:border-graphite-500 text-xs font-mono tracking-wider transition-all cursor-pointer rounded-md bg-graphite-800/40"
              title={isZh ? "Switch to English" : "切换为中文"}
              aria-label="Toggle language"
            >
              {currentLocale.toUpperCase() === "EN" ? "中文" : "EN"}
            </button>

            {/* Schedule Pass CTA */}
            <Link
              href={`/${currentLocale}/booking`}
              className="px-4 py-2 bg-signal hover:bg-signal-soft text-graphite text-xs font-bold uppercase tracking-wider shadow-lg shadow-signal/20 transition-all hover:scale-105 active:scale-95 rounded-md whitespace-nowrap"
            >
          ...
          <div className="flex items-center gap-2.5 lg:hidden">
            {/* Compact Quick Language Button */}
            <button
              type="button"
              onClick={toggleLocale}
              className="px-2.5 py-1.5 border border-graphite-600 text-steel-2 hover:text-white text-xs font-mono rounded-md cursor-pointer active:bg-white/10"
            ...
            <button
              ...
              className="p-2.5 text-steel-2 hover:text-white focus:outline-none focus:ring-2 focus:ring-signal rounded-md bg-graphite-800/80 border border-graphite-600/80 transition-colors cursor-pointer active:scale-95"
